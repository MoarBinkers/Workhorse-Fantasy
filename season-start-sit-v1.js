...
function detailCard(x){
 ...
 const currentRows=[['2026 PPG',fmt(x.seasonWork?.ppg,1)],['Last 3 PPG',fmt(x.currentWork?.ppg,1)],['Latest actual role',x.latestRole?.score==null?'—':`${x.latestRole.score}/100`],['Expected role after confirmed news',x.forwardRoleScore==null?'—':`${x.forwardRoleScore}/100`],['Latest snap share',pct(x.latestRole?.snap)],['Latest rush share',pct(x.latestRole?.rushShare)],['Latest target share',pct(x.latestRole?.targetShare)],['Latest red-zone share',pct(x.latestRole?.rzShare)],['Carries / G',fmt(x.currentWork?.carries,1)],['Targets / G',fmt(x.currentWork?.targets,1)],['Routes / G',fmt(x.currentWork?.routes,1)],['Red-zone looks / G',fmt(x.currentWork?.rz,1)]];
