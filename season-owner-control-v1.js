...
let authorized=false,enabled=true,queued=false,rankObserver=null,observedBox=null;
...
function attachRankObserver(){const box=document.querySelector('#rank-rows');if(box===observedBox)return;rankObserver?.disconnect();observedBox=box||null;rankObserver=null;if(!observedBox)return;rankObserver=new MutationObserver(()=>{if(enabled)queue()});rankObserver.observe(observedBox,{childList:true,subtree:false});if(enabled)queue()}
async function start(){authorized=await owner();if(!authorized)return;enabled=storedMode();styles();setEnabled(enabled);queue();attachRankObserver();const barObs=new MutationObserver(mutations=>{attachRankObserver();const touchesRanks=!!observedBox&&mutations.some(m=>m.target===observedBox||m.target.closest?.('#rank-rows'));if(!document.querySelector('#wh-owner-mode-toggle')||touchesRanks)queue()});barObs.observe(document.body,{childList:true,subtree:true});setTimeout(()=>{attachRankObserver();queue()},250);setTimeout(()=>{attachRankObserver();queue()},900)}
start();
})();