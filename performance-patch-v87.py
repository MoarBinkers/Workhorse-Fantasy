...
# Player clicks must never wait for a full historical database crawl.
replace_exact(
    'central-adp-v36.js',
...
# Keep only the currently-open player's bridge history in localStorage.
replace_exact(
    'central-adp-v36.js',
...
# Open player drawers immediately; hydrate history only after the visible content opens.
replace_exact(
    'central-adp-v36.js',
...
# Ordinary renders should not rebuild the full central market and player directory.
replace_exact(
    'central-adp-v36.js',
...
# Identity enrichment can repaint later, during idle time, rather than interrupting a click.
replace_exact(
    'central-adp-v36.js',
...
# Remove timed full-app rerenders that can randomly collide with early clicks.
replace_exact(
    'central-adp-v36.js',
...
# Cap graph DOM complexity and do graph work only after the drawer can paint.
replace_exact(
    'player-detail-v34.js',
...
# Cache-bust changed production scripts.
replace_exact('index.html', './player-detail-v34.js?v=342', './player-detail-v34.js?v=343')
replace_exact('index.html', './central-adp-v36.js?v=366', './central-adp-v36.js?v=367')
...
assert ".order('captured_at',{ascending:false}).limit(240)" in central
assert '.range(from,from+999)' not in central
assert 'await hydrateHistory(p)' not in central
assert 'if(activeRows.length)applyRows(activeRows,activeDirectory);return originalRender' not in central
assert '[1200,3000].forEach' not in central
assert 'WorkhorseRefreshPlayerHistory' in detail
assert 'raw.length>120' in detail
assert './central-adp-v36.js?v=367' in index
assert './player-detail-v34.js?v=343' in index
print('Interaction performance patch assertions passed.')
