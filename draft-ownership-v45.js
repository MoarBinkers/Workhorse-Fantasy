// v45.3 — completed-pick ownership follows Workhorse traded draft capital before Sleeper roster fallbacks.
(()=>{
  ...
  function tradedSchedule(slot=selectedSlot()){
    if(!slot)return null;
    try{
      const api=window.WorkhorseDraftTradeCapital,c=api?.capital?.();
      if(!c||Number(c.slot)!==Number(slot)||!Array.isArray(c.owned))return null;
      const hasTrade=(Array.isArray(c.acquired)&&c.acquired.length>0)||(Array.isArray(c.away)&&c.away.length>0)||(api?.pickOverrides?.()?.length>0);
      if(!hasTrade)return null;
      return new Set(c.owned.map(x=>Number(x.pickNo)).filter(n=>n>0));
    }catch(_){return null}
  }
  function isOwnPick(p,slot){
    if(!slot||!p)return false;
    const schedule=tradedSchedule(slot),pickNo=Number(p.pick_no)||0;
    // For drafts with Workhorse trade ownership, pick number is authoritative.
    // Some completed Sleeper mock/draft payloads keep roster_id aligned to the
    // original board slot, which would incorrectly reclaim traded-away picks.
  ...
  async function tick(){if(busy)return;busy=true;try{if(!draftId&&!(await resolveDraft(false))){renderOwnPicks();return}const latest=await sleeper('https://api.sleeper.app/v1/draft/'+draftId+'/picks');if(!Array.isArray(latest))throw new Error('Invalid Sleeper picks response');const next=signature(latest),changed=next!==lastSig;picks=latest;lastSig=next;if(changed){try{await window.WorkhorseDraftTradeCapital?.refresh?.()}catch(_){};try{if(typeof window.pollDraft==='function')await window.pollDraft()}catch(_){}}renderOwnPicks();reconcile();status('● Live sync · Sleeper picks checked about every 1 second');if(changed)renderOwnPicks()}catch(e){status('Live sync reconnecting automatically…',true);if(Date.now()-lastResolve>4000){draftId='';draftMeta=null}}finally{busy=false}}
