...
function clearTierTarget(){document.querySelectorAll('.wh-ros-tier-break.wh-tier-drop-active').forEach(x=>x.classList.remove('wh-tier-drop-active'));if(drag)drag.tierTarget=null}
...
function chooseTarget(y){
 if(!drag)return;
 if(!isWeekly){
  const tier=[...document.querySelectorAll('#rank-rows > .wh-ros-tier-break[data-tier-letter]')].find(x=>{const r=x.getBoundingClientRect();return x.offsetParent!==null&&y>=r.top&&y<=r.bottom});
  if(tier){
   clearTierTarget();tier.classList.add('wh-tier-drop-active');drag.tierTarget={tier:tier.dataset.tierLetter||'',position:tier.dataset.position||''};drag.target=null;
   const rect=tier.getBoundingClientRect(),board=document.querySelector('#rank-rows')?.getBoundingClientRect(),l=line();l.style.display='block';l.style.left=Math.max(8,(board?.left||rect.left)+4)+'px';l.style.width=Math.max(40,(board?.width||rect.width)-8)+'px';l.style.top=Math.round(rect.bottom)+'px';return;
  }
 }
 clearTierTarget();
 const rows=visibleRows().filter(r=>String(r.dataset.id)!==drag.id);if(!rows.length){drag.target=null;line().style.display='none';return}let target=rows[rows.length-1],after=true;for(const row of rows){const r=row.getBoundingClientRect();if(y<r.top+r.height/2){target=row;after=false;break}}
...
function finish(commit=true){
 if(!drag)return;const d=drag;drag=null;cancelAnimationFrame(raf);raf=0;document.documentElement.classList.remove('wh-sorting');d.source?.classList.remove('wh-drag-source');d.ghost?.remove();document.querySelectorAll('.wh-ros-tier-break.wh-tier-drop-active').forEach(x=>x.classList.remove('wh-tier-drop-active'));const l=document.querySelector('#wh-drop-line');if(l)l.style.display='none';try{d.handle?.releasePointerCapture?.(d.pid)}catch(_){ }
 if(!commit)return;
 if(d.tierTarget?.tier&&d.tierTarget?.position){window.dispatchEvent(new CustomEvent('workhorse:ros-tier-drop',{detail:{playerId:d.id,tier:d.tierTarget.tier,position:d.tierTarget.position}}));return}
 if(d.target){window.dispatchEvent(new CustomEvent('workhorse:rank-dragged',{detail:{playerId:d.id,targetId:d.target,after:!!d.after}}));const arr=loadOrder();const sourceIndex=arr.indexOf(d.id);if(sourceIndex>=0){const remaining=arr.filter(x=>x!==d.id);const ti=remaining.indexOf(d.target);if(ti>=0){const finalIndex=ti+(d.after?1:0);invokeMove(d.id,Math.max(0,Math.min(arr.length-1,finalIndex)))}}}
}
function begin(e,handle,row){const id=String(row.dataset.id||'');if(!id)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();neutralize();const ghost=makeGhost(row);drag={pid:e.pointerId,id,source:row,handle,ghost,target:null,after:false,tierTarget:null,x:e.clientX,y:e.clientY};row.classList.add('wh-drag-source');document.documentElement.classList.add('wh-sorting');try{handle.setPointerCapture(e.pointerId)}catch(_){ }placeGhost(e.clientX,e.clientY);chooseTarget(e.clientY);if(!raf)raf=requestAnimationFrame(scrollLoop)}
...
function ensureOwnerControl(){if(window.__WH_OWNER_CONTROL_V1__||document.querySelector('script[data-wh-owner-control-loader]'))return;const s=document.createElement('script');s.src='./season-owner-control-v1.js?v=1';s.async=false;s.dataset.whOwnerControlLoader='1';document.documentElement.appendChild(s)}
function start(){styles();ensureOwnerControl();neutralize();const obs=new MutationObserver(neutralize);obs.observe(document.documentElement,{childList:true,subtree:true});document.addEventListener('pointerdown',onDown,true);document.addEventListener('pointermove',onMove,{capture:true,passive:false});document.addEventListener('pointerup',onUp,true);document.addEventListener('pointercancel',onCancel,true);document.addEventListener('dragstart',e=>{if(e.target.closest?.('#rank-rows')){e.preventDefault();e.stopImmediatePropagation()}},true);document.addEventListener('keydown',e=>{if(e.key==='Escape'&&drag)finish(false)},true)}
start();
})();