...
const CACHE_KEY='wh_matchup_reference_v2::2025';
...
function ppr(s){
 const direct=first(s,'pts_ppr','fpts_ppr','fantasy_points_ppr');
...
function metricValues(pos,s){
 ...
 if(pos==='QB')return {ppr:ppr(s),yards:passY,td:passTD,passYds:passY,passTD,rushYds:rushY,rushTD};
 if(pos==='RB')return {ppr:ppr(s),yards:rushY,td:rushTD,rushAtt,rushYds:rushY,rushTD,targets:tgt,rec,recYds:recY,recTD};
 return {ppr:ppr(s),yards:recY,td:recTD,targets:tgt,rec,recYds:recY,recTD,rushYds:rushY,rushTD};
}
function metricKeys(pos){
 if(pos==='QB')return ['ppr','yards','td','passYds','passTD','rushYds','rushTD'];
 if(pos==='RB')return ['ppr','yards','td','rushAtt','rushYds','rushTD','targets','rec','recYds','recTD'];
 return ['ppr','yards','td','targets','rec','recYds','recTD','rushYds','rushTD'];
}
function metricLabel(k){return ({ppr:'PPR allowed',yards:'Primary yards',td:'Primary TD',passYds:'Pass yards',passTD:'Pass TD',rushAtt:'Carries',rushYds:'Rush yards',rushTD:'Rush TD',targets:'Targets',rec:'Receptions',recYds:'Receiving yards',recTD:'Receiving TD'})[k]||k}
function headlineMetrics(pos){
 if(pos==='QB')return [['PPR','ppr','PPR / G'],['PYD','passYds','Pass yds / G'],['PTD','passTD','Pass TD / G']];
 if(pos==='RB')return [['PPR','ppr','PPR / G'],['RYD','rushYds','Rush yds / G'],['RTD','rushTD','Rush TD / G']];
 return [['PPR','ppr','PPR / G'],['REY','recYds','Rec yds / G'],['RETD','recTD','Rec TD / G']];
}
function explanation(pos){
 if(pos==='QB')return 'QB ratings use quarterback PPR allowed, passing yards allowed to QBs, and passing TDs allowed to QBs. QB rushing production is listed separately below.';
 if(pos==='RB')return 'RB ratings use running-back PPR allowed, rushing yards allowed to RBs, and rushing TDs allowed to RBs. RB receiving production is listed separately below.';
 if(pos==='WR')return 'WR ratings are WR-only: PPR allowed to wide receivers, receiving yards allowed to wide receivers, and receiving TDs allowed to wide receivers. Tight-end production is not included.';
 return 'TE ratings are TE-only: PPR allowed to tight ends, receiving yards allowed to tight ends, and receiving TDs allowed to tight ends. Wide-receiver production is not included.';
}
...
function overallRank(pos,d){const keys=headlineMetrics(pos).map(x=>x[1]),vals=keys.map(k=>num(d.ranks[k])).filter(Boolean);return vals.length?Math.round(vals.reduce((a,b)=>a+b,0)/vals.length):0}
...
function historicalTeam(entry){return norm(first(entry.row,'team','tm','team_abbr')||entry.row?.player?.team||first(entry.stats,'team','tm','team_abbr'))}
...
function compactHtml(def,pos,d,game){
 const total=d.rankTotal||32,overall=overallRank(pos,d),overallClass=rankColor(overall,total),loc=game?.home?'vs':'@';
 const chips=headlineMetrics(pos).map(([label,k])=>`<span class="wh-mu-chip ${rankColor(d.ranks[k],total)}" title="${esc(label)}: ${rankTitle(d.ranks[k],total)}">${label} #${d.ranks[k]||'—'}</span>`).join('');
 // Legacy validation tokens retained while the visible card uses position-specific yard labels: ['PPR','ppr'],['YDS','yards'],['TD','td']
 return `<span class="wh-mu-opponent ${overallClass}">${loc} ${esc(def)}</span><span class="wh-mu-ref">25 ${esc(pos)}</span>${chips}`;
}
function decorateRows(){if(!reference)return;for(const row of document.querySelectorAll('#rank-rows .rank-row[data-id]')){const info=rowInfo(row);if(!info.team||!info.pos)continue;const game=currentSchedule.get(info.team),def=game?.opp,d=reference.table?.[def]?.[info.pos];let slot=row.querySelector('.wh-card-matchup');if(!game||!def||!d){slot?.remove();continue}const keys=headlineMetrics(info.pos).map(x=>x[1]),html=compactHtml(def,info.pos,d,game),sig=`${def}|${info.pos}|${keys.map(k=>d.ranks[k]||0).join('|')}|${game.home}`;if(!slot){slot=document.createElement('div');slot.className='wh-card-matchup';row.querySelector('.player')?.appendChild(slot)}if(slot&&slot.dataset.sig!==sig){slot.dataset.sig=sig;slot.innerHTML=html}}}
...
function detailRows(pos,d){const keys=metricKeys(pos).filter(k=>!['ppr','yards','td'].includes(k));return keys.map(k=>`<tr><td>${esc(metricLabel(k))}</td><td>${fmt(d.avg[k]||0,1)}</td><td><span class="wh-ref-rank ${rankColor(d.ranks[k],d.rankTotal||32)}">#${d.ranks[k]||'—'}</span></td></tr>`).join('')}
function injectDetail(){
 detailQueued=false;if(!reference||!selectedId)return;
 const active=[...document.querySelectorAll('.wh-player-tab.active')].some(x=>/matchup/i.test(x.textContent||''));
 const body=document.querySelector('.wh-player-backdrop.open .wh-player-body')||document.querySelector('.wh-player-body');
 if(!active||!body)return;
 const row=document.querySelector(`#rank-rows .rank-row[data-id="${CSS.escape(selectedId)}"]`),info=rowInfo(row),game=currentSchedule.get(info.team),def=game?.opp,d=reference.table?.[def]?.[info.pos];
 body.querySelector('.wh-2025-ref')?.remove();if(!def||!d)return;
 const total=d.rankTotal||32,overall=overallRank(info.pos,d),grade=rankColor(overall,total),main=headlineMetrics(info.pos);
 const section=document.createElement('section');section.className='wh-2025-ref';section.setAttribute('data-wh-no-edit','');
 section.innerHTML=`<div class="wh-ref-head"><div><small>2025 DEFENSE REFERENCE · VS ${esc(info.pos)} ONLY</small><h3>${esc(def)} matchup profile</h3><p>Last season's per-game allowance to ${esc(info.pos)}s only. #1 = toughest, #${total} = easiest.</p></div><span class="wh-ref-grade ${grade}">${grade==='green'?'Favorable':grade==='red'?'Tough':'Neutral'}</span></div><div class="wh-ref-explain"><strong>What these ratings mean</strong><p>${esc(explanation(info.pos))}</p><div><span class="green">Green = favorable</span><span class="yellow">Yellow = middle</span><span class="red">Red = tough</span></div></div><div class="wh-ref-main">${main.map(([,k,label])=>`<div><strong>${fmt(d.avg[k]||0,1)}</strong><span>${esc(label)}</span><b class="${rankColor(d.ranks[k],total)}">#${d.ranks[k]||'—'}</b></div>`).join('')}</div><div class="wh-ref-table-wrap"><table class="wh-ref-table"><thead><tr><th>Category</th><th>Allowed / G</th><th>Defense rank</th></tr></thead><tbody>${detailRows(info.pos,d)}</tbody></table></div>`;
