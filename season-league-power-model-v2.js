...
// Historical PPR rank-band shapes calibrate how much one lineup slot usually separates
// from replacement. They are not current-player projections. Workhorse ROS remains the
// opinion layer that places every player in the overall + positional order.
const HIST={
...
const START_EDGE_SCALE={QB:2.0,RB:2.2,WR:2.2,TE:2.1};
const START_DEFICIT_SCALE={QB:1.2,RB:1.6,WR:1.5,TE:1.4};
...
// ASSET VALUE: driven mostly by the user's overall Workhorse ROS board. Adjacent overall
// ranks stay close. This is useful for depth / injury insurance and the audit display.
...
// STARTING-SLOT VALUE: team POWER must answer "how much does this player improve ONE
// legal lineup spot?" rather than "how valuable is this player as an asset?". Every
// starter begins near the same baseline. Historical positional edge creates a measured
// advantage above/below that baseline, and overall ROS only adds a small nudge.
function lineupRosNudge(overallRank){const r=Math.max(1,finite(overallRank,999));return 4*Math.exp(-.012*(r-1))}
function starterSlotValue(ctx,pos,overallRank,posRank){
  const hist=historicalPpg(pos,posRank),repl=historicalPpg(pos,replacementRank(ctx,pos)),delta=hist-repl;
  const edge=Math.max(0,delta)*(START_EDGE_SCALE[pos]||2.1),deficit=Math.max(0,-delta)*(START_DEFICIT_SCALE[pos]||1.4);
  return Math.max(68,Math.min(112,finite(82+edge-deficit+lineupRosNudge(overallRank),82)));
}
...
function benchWeight(ctx,p,index){const n=leagueSize(ctx),f=flexCounts(ctx);if(p.position==='QB'){
  ...
  if(p.position==='RB'||p.position==='WR')return[.24,.14,.08,.05,.035,.025][index]??.018;
  return .01;
}
...
// Mild lineup-balance pressure: the lower half of the legal starting lineup gets a
// little extra say in the team result. It is intentionally mild; it cannot override
// the player's actual Workhorse rank, but it stops two stars from hiding several holes.
function lineupBalance(starters){
  ...
  const lineupScore=starterSum*.88+(lowerHalfAvg*values.length)*.12,factor=starterSum>0?Math.max(.88,Math.min(1,lineupScore/starterSum)):1;
...
window.WorkhorsePowerModelV2={POS,HIST,finite,leagueSize,directSlots,flexCounts,replacementRank,historicalPpg,rosBase,scarcityAdjustment,playerValue,lineupRosNudge,starterSlotValue,lineupBalance,metric,buildTeams,offenseStarterCount,benchSlotCount};
})();