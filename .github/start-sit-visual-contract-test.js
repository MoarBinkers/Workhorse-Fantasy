'use strict';
const fs=require('fs');
const s=fs.readFileSync('season-start-sit-v1.js','utf8');

const must=[
  'id="wh-startsit"',
  '<style id="wh-startsit-css">',
  '#wh-startsit .compare-panel{',
  '#wh-startsit .compare-grid{',
  '#wh-startsit .compare-card{',
  '#wh-startsit .compare-row{',
  '#wh-startsit .compare-value{',
  '#wh-startsit .propstack',
  '#wh-startsit .matchstack',
  'grid-template-columns:repeat(auto-fit,minmax(300px,1fr))',
  'function safeMatrixCell(row,x)',
  'function renderMatrix(graded)',
  'function renderDetails(graded)',
  'styles();shell();bind();'
];
for(const m of must)if(!s.includes(m))throw new Error('Start/Sit visual contract missing: '+m);

const bad=[
  ':root{','html,body{','body{margin:','function extraStyles()',
  'font-size:6px','font-size:7px','font-size:8px',
  'class="matrix"','<table class="matrix"','#wh-startsit .matrix{',
  'table-layout:fixed','min-width:760px'
];
for(const m of bad)if(s.includes(m))throw new Error('Start/Sit visual regression detected: '+m);

const css=(s.match(/<style id="wh-startsit-css">([\s\S]*?)<\/style>/)||[])[1]||'';
if(!css)throw new Error('Scoped Start/Sit stylesheet missing');
for(const selector of ['.top{','.shell{','.intro{','.setup{','.compare-grid{','.compare-card{','.compare-row{','.details-grid{']){
  if(!css.includes('#wh-startsit '+selector))throw new Error('Critical selector is not Start/Sit scoped: '+selector);
}
console.log('PASS: Start/Sit stable card layout');
