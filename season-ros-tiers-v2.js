(()=>{
'use strict';
if(window.__WH_ROS_TIERS_V2__)return;
window.__WH_ROS_TIERS_V2__=true;
...
function initState(){
 ...
 for(const id of order()){const pos=positionById.get(id);if(!pos)continue;counts[pos]++;assignment[id]=defaultTier(pos,counts[pos])}
 state={version:2,defs,assignment};save();
}
function ensureAssignments(){
 ...
 for(const id of order()){
  ...
  const defs=state.defs[pos]||['A','B','C','D'],fallback=defaultTier(pos,counts[pos]);
  if(!state.assignment[id]){state.assignment[id]=defs.includes(fallback)?fallback:defs[defs.length-1];changed=true}
...
function styles(){if(document.querySelector('#wh-ros-tier-v2-css'))return;const s=document.createElement('style');s.id='wh-ros-tier-v2-css';s.textContent=`
html.wh-ros-tier-system #rank-rows>.wh-tier-break{display:none!important}
...
function clearEditableBreaks(){document.querySelectorAll('#rank-rows > .wh-ros-tier-break').forEach(x=>x.remove())}
function renderAll(){clearEditableBreaks();for(const row of document.querySelectorAll('#rank-rows .rank-row[data-id]'))addTierChip(row,String(row.dataset.id||''))}
function makeBreak(pos,tier,count){const d=document.createElement('div');d.className='wh-ros-tier-break';d.dataset.tierLetter=tier;d.dataset.position=pos;d.dataset.whNoEdit='';d.innerHTML=`<strong>Tier ${esc(tier)}</strong><small>${count?`${count} player${count===1?'':'s'}`:'Empty · drop a player here'}</small>`;return d}
function renderPosition(pos){
 clearEditableBreaks();const rows=visiblePositionRows(pos),defs=state.defs[pos]||['A','B','C','D'];for(const row of rows)addTierChip(row,String(row.dataset.id||''));
 let anchor=null;
 for(let i=defs.length-1;i>=0;i--){const tier=defs[i],members=rows.filter(r=>tierFor(r.dataset.id)===tier),d=makeBreak(pos,tier,members.length),before=members[0]||anchor,box=document.querySelector('#rank-rows');if(before)box.insertBefore(d,before);else box.appendChild(d);anchor=d}
 document.querySelector('#rank-rows > .wh-ros-tier-break')?.classList.add('first');
}
function ensureAddTierButton(){
 const actions=document.querySelector('.toolbar')||document.querySelector('.hero-actions');if(!actions)return;let b=document.querySelector('#wh-ros-add-tier');
 if(!b){b=document.createElement('button');b.id='wh-ros-add-tier';b.className='wh-ros-add-tier';b.type='button';b.dataset.whNoEdit='';b.addEventListener('click',()=>{const pos=activeFilter();if(!POSITIONS.includes(pos)||!ownerOn())return;const defs=state.defs[pos],l=nextLetter(defs);defs.push(l);save();queue();toast(`${pos} Tier ${l} added.`)})}
 if(b.parentElement!==actions)actions.appendChild(b);const pos=activeFilter();b.hidden=!ownerOn();b.disabled=!POSITIONS.includes(pos);b.textContent=POSITIONS.includes(pos)?`+ ${pos} TIER`:'+ TIER';b.title=POSITIONS.includes(pos)?`Add another ${pos} tier`:'Choose QB, RB, WR, or TE first';
}
...
function moveToFinalIndex(sourceId,finalIndex){const arr=order(),i=arr.indexOf(String(sourceId));if(i<0||finalIndex<0||finalIndex>=arr.length||i===finalIndex)return;const row=rowFor(sourceId),input=row?.querySelector(`[data-rank="${CSS.escape(String(sourceId))}"]`);if(!input)return;if(i<finalIndex&&finalIndex===arr.length-1){input.value=String(arr.length);input.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(()=>rowFor(sourceId)?.querySelector(`[data-down="${CSS.escape(String(sourceId))}"]`)?.click(),0);return}const requested=i<finalIndex?finalIndex+2:finalIndex+1;input.value=String(requested);input.dispatchEvent(new Event('change',{bubbles:true}))}
function desiredIndexForTier(sourceId,pos,tier){
 const arr=order(),remaining=arr.filter(x=>x!==String(sourceId)),defs=state.defs[pos]||[],targetMembers=remaining.filter(id=>positionById.get(id)===pos&&tierFor(id)===tier);if(targetMembers.length)return remaining.indexOf(targetMembers[0]);const ti=tierIndex(pos,tier);
 for(let j=ti+1;j<defs.length;j++){const next=remaining.find(id=>positionById.get(id)===pos&&tierFor(id)===defs[j]);if(next)return remaining.indexOf(next)}
 let last=-1;for(let j=0;j<remaining.length;j++)if(positionById.get(remaining[j])===pos&&tierIndex(pos,tierFor(remaining[j]))<=ti)last=j;return last>=0?Math.min(remaining.length,last+1):Math.max(0,remaining.length-1)
}
...
function handleTierDrop(e){const {playerId,tier,position}=e.detail||{};if(!ownerOn()||!playerId||!tier||!position)return;const pos=positionById.get(String(playerId));if(pos!==position)return;const finalIndex=desiredIndexForTier(playerId,pos,tier);if(!assignTier(playerId,tier))return;moveToFinalIndex(playerId,finalIndex);toast(`${rowFor(playerId)?.querySelector('.name')?.textContent?.trim()||'Player'} → ${pos} Tier ${tier}`);setTimeout(queue,40)}
function handleNormalDrag(e){const {playerId,targetId}=e.detail||{},f=activeFilter();if(!ownerOn()||!POSITIONS.includes(f)||!playerId||!targetId)return;const p1=positionById.get(String(playerId)),p2=positionById.get(String(targetId));if(p1!==f||p2!==f)return;const tier=tierFor(targetId);if(tier&&tierFor(playerId)!==tier){assignTier(playerId,tier);setTimeout(queue,30)}}
function rowMutation(m){for(const n of [...m.addedNodes,...m.removedNodes])if(n?.nodeType===1&&(n.matches?.('.rank-row')||n.querySelector?.('.rank-row')))return true;return false}

async function start(){
 document.documentElement.classList.add('wh-ros-tier-system');styles();await loadPositions();initState();ensureAssignments();queue();
 window.addEventListener('workhorse:ros-tier-drop',handleTierDrop);window.addEventListener('workhorse:rank-dragged',handleNormalDrag);window.addEventListener('workhorse:owner-mode-changed',()=>setTimeout(queue,0));
 document.addEventListener('click',e=>{if(e.target.closest?.('[data-filter]'))setTimeout(queue,35)},true);
 const box=document.querySelector('#rank-rows');if(box){const obs=new MutationObserver(muts=>{if(!busy&&muts.some(rowMutation))queue()});obs.observe(box,{childList:true,subtree:true})}
}
start();
})();